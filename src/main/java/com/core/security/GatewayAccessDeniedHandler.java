...
public class GatewayAccessDeniedHandler extends AbstractOAuth2SecurityExceptionHandler implements AccessDeniedHandler {
    ...
    public GatewayAccessDeniedHandler(ExceptionRenderer exceptionRenderer) {
    ...
    protected void handleHttpEntityResponse(ResponseEntity<OAuth2Exception> result, HttpServletResponse response,
        ...
        try {
            if (OAuth2Exception.ACCESS_DENIED.equalsIgnoreCase(result.getBody().getOAuth2ErrorCode())) {
                exception = new IllegalAccessException(result.getBody().getMessage());
            ...
            exception = new GatewayException("未知错误");
        } finally {
